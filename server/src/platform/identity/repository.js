export function createIdentityRepository(pool) {
  return {
    ...
    create(values) { return pool.query('INSERT INTO users(uuid,code,name,password_hash,role,avatar) VALUES($1,$2,$3,$4,$5,$6) RETURNING *', values).then((result) => result.rows[0]) },
    update(id, values) { return pool.query('UPDATE users SET name=$1,role=$2,password_hash=COALESCE($3,password_hash),avatar=$4,updated_at=NOW() WHERE id=$5', [...values, id]) },
