export function createSettingsService(repository, imageStore, mappers, securityPolicy) {
  ...
  return {
    ...
    async updateSystem(body) {
      ...
      const systemLogo = body.systemLogo === undefined ? { display: current.system_logo, original: current.system_logo_original || current.system_logo } : await imageStore.persist(body.systemLogo, current.system_logo, current.system_logo_original)
      const titleLogo = body.titleLogo === undefined ? { display: current.title_logo, original: current.title_logo_original || current.title_logo } : await imageStore.persist(body.titleLogo, current.title_logo, current.title_logo_original)
      const result = await repository.updateSystem([systemTitle, browserTitle, systemLogo.display, systemLogo.original, titleLogo.display, titleLogo.original, text(body.loginText, current.login_text, 255), text(body.footerRecord, current.footer_record || '', 5000) || null, typeof body.showWorkbenchHeader === 'boolean' ? body.showWorkbenchHeader : current.show_workbench_header, typeof body.showAiChatHeader === 'boolean' ? body.showAiChatHeader : current.show_ai_chat_header])
