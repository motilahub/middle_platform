...
- 知识库和智能体选择器嵌入聊天输入框左下角，输入区与消息内容保持同一聊天工作区；
- 支持后台配置智能体的 System Prompt、模型和通用参数；
- 支持后台配置聊天主题（明亮、深色、自然）、动态效果开关和 CHAT Header 显示状态；
- CHAT 左侧品牌区域复用系统配置中的 Logo 和系统标题，右侧 Header 可独立开关；
- 文本消息持久化；
- 除独立 Chat 页面外，所有已登录平台页面右下角显示 AI Chat 悬浮入口；
- 使用 OpenAI Chat Completions 兼容协议生成回复；
- 通过 SSE 增量返回回复，支持加载、错误和空状态；
- 支持文件、图片附件上传，附件消息持久化并在消息流中预览/下载；
- 支持浏览器语音识别输入，以及 `@` 文件、知识库、智能体和 `/` Skill 快捷搜索；
...
推荐问题策略：新会话从起始话题池中随机抽取配置数量的问题；已有会话根据最近一条有效消息的内容和关键词（计划、代码、数据、总结等）派生后续问题，避免所有会话显示相同的固定文案。
...
- 输入框支持 `@` 文件、知识库、智能体和 `/` Skill 建议；选择知识库后，本轮消息使用该知识库检索上下文。
- 浏览器语音录入转文字；
- 已预留取消生成和重新生成的交互位置。
