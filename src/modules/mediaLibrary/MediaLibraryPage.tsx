  ...
  return <MediaThemeProvider darkMode={darkMode}><div className={`media-library-page${darkMode ? ' is-dark' : ''}`}>
    ...
    <main className="media-library-main">
      <div className="media-library-filters">
        <Segmented<MediaType> value={type} options={[{ label: '电影', value: 'movie' }, { label: '电视剧', value: 'tv' }]} onChange={(value) => { setType(value); setPage(1) }} />
        <Input.Search allowClear value={searchText} prefix={<SearchOutlined />} placeholder="搜索影视名称" enterButton="搜索" onChange={(event) => { setSearchText(event.target.value); if (!event.target.value) { setQuery(''); setPage(1) } }} onSearch={(value) => { setQuery(value.trim()); setPage(1) }} />
      </div>
      {loading
        ...
        : items.length
          ? <><section ref={listRef} className="media-library-grid" aria-label={type === 'movie' ? '电影榜单' : '电视剧榜单'}>
              {pageItems.map((item) => <button type="button" className="media-library-item" key={item.id} onClick={() => navigate(`/media-library/${item.id}`)}>
            ...
            <Pagination className="media-library-pagination" current={page} pageSize={PAGE_SIZE} total={items.length} showSizeChanger={false} hideOnSinglePage onChange={changePage} /></>
