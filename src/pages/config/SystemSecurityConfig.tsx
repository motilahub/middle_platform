...
export default function SystemSecurityConfig() {
  ...
  const originalField = (field: 'systemLogo' | 'titleLogo') => field === 'systemLogo' ? 'systemLogoOriginal' : 'titleLogoOriginal'
  const imageControl = (field: 'systemLogo' | 'titleLogo', label: string) => <Form.Item label={label} extra="支持 PNG、JPG、WebP 等图片格式，最大 2MB">
    <Form.Item noStyle shouldUpdate={(before, after) => before[field] !== after[field]}>{() => {
      ...
      return <Upload accept="image/*" beforeUpload={selectImage(field)} showUploadList={false}>{value ? <div className="image-upload-card"><img src={value} alt={label} /><div className="image-action image-download" title="下载原图" onClick={(event) => { event.preventDefault(); event.stopPropagation(); downloadImage(form.getFieldValue(originalField(field)) || value, field) }}><DownloadOutlined /></div><div className="image-action image-delete" title="删除图片" onClick={(event) => { event.preventDefault(); event.stopPropagation(); form.setFieldsValue({ [field]: null, [originalField(field)]: null }) }}><DeleteOutlined /></div></div> : <div className="image-upload-card image-upload-empty" title="选择图片"><UploadOutlined /></div>}</Upload>
    }}</Form.Item>
