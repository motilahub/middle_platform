...
export default function ConfigLayout({ onLogout }: { onLogout: () => void }) {
  ...
  const selectedKey = location.pathname.includes('/config/media-library') ? 'media-library' : location.pathname.includes('/model-providers') ? 'model-providers' : location.pathname.includes('/ai-agents') ? 'ai-agents' : location.pathname.includes('/ai-chat') ? 'ai-chat' : location.pathname.includes('/permission-groups') ? 'permission-groups' : location.pathname.includes('/app-categories') ? 'app-categories' : location.pathname.includes('/basic-config') ? 'basic-config' : location.pathname.includes('/system-security') ? 'system-security' : location.pathname.includes('/sso/inbound') ? 'sso-inbound' : location.pathname.includes('/sso/outbound') ? 'sso-outbound' : location.pathname.includes('/sso/access-logs') ? 'sso-access-logs' : location.pathname.includes('users') ? 'users' : location.pathname.includes('dashboard') ? 'dashboard' : 'workbench'
  const ssoMenu = can('platform.sso.read') ? { key: 'sso-config', label: '单点登录', children: [{ key: 'sso-inbound', label: '外部访入', onClick: () => navigate('/config/sso/inbound') }, { key: 'sso-outbound', label: '内部访出', onClick: () => navigate('/config/sso/outbound') }, { key: 'sso-access-logs', label: '访问日志', onClick: () => navigate('/config/sso/access-logs') }] } : null
  const menuItems = [
  ...
  const brand = <button type="button" className="sider-brand" onClick={() => navigate('/')} aria-label="返回首页"><img className="brand-mark small" src={settings.systemLogo || defaultLogo} alt={settings.systemTitle} />{(!collapsed || !screens.md) && <strong>{settings.systemTitle}</strong>}</button>
