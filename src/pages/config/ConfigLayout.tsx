...
export default function ConfigLayout({ onLogout }: { onLogout: () => void }) {
  ...
  const menuItems = [
    ...
    (can('platform.app.read') || can('platform.settings.read') || can('platform.permission.read') || can('platform.model_provider.read') || ssoMenu) && { key: 'system-config', icon: <SettingOutlined />, label: '系统配置', children: [can('platform.settings.read') && { key: 'basic-config', label: '基础配置', onClick: () => navigate('/config/basic-config') }, can('platform.settings.read') && { key: 'system-security', label: '系统安全', onClick: () => navigate('/config/system-security') }, can('platform.app.read') && { key: 'app-categories', label: '控制台分类', onClick: () => navigate('/config/app-categories') }, can('platform.model_provider.read') && { key: 'model-providers', label: '模型供应商', onClick: () => navigate('/config/model-providers') }, can('platform.permission.read') && { key: 'permission-groups', label: '权限管理', onClick: () => navigate('/config/permission-groups') }, ssoMenu].filter(Boolean) },
  ].filter(Boolean)
