...
export default function SsoConfig({ direction }: { direction: SsoDirection }) {
  ...
  return <div>
    ...
    <Table className="config-table" loading={loading} rowKey="id" columns={columns} dataSource={rows.filter((row) => `${row.code} ${row.name} ${row.systemUrl}`.toLowerCase().includes(keyword.trim().toLowerCase()))} scroll={{ x: 1300 }} rowSelection={{ selectedRowKeys: checked, preserveSelectedRowKeys: true, onChange: (keys) => setChecked(keys as number[]), getCheckboxProps: () => ({ disabled: !can('platform.sso.unlink') }) }} pagination={{ current: page.current, pageSize: page.pageSize, showSizeChanger: true, pageSizeOptions: [10, 20, 50], showTotal: (total) => `共 ${total} 条`, onChange: (current, pageSize) => setPage({ current, pageSize }) }} onRow={(record) => ({ onClick: (event) => { if ((event.target as HTMLElement).closest('button,.ant-popover,.ant-switch,.ant-checkbox-wrapper')) return; openEdit(record) } })} />
