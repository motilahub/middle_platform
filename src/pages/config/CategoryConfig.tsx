...
export default function CategoryConfig() {
  ...
  const remove = async (category: DashboardCategory) => {
    try { await api.deleteCategory(category.id); setChecked((ids) => ids.filter((id) => id !== category.id)); await load(); message.success('已删除，关联应用已归入未分类') } catch (error) { message.error((error as Error).message) }
  }
  const batchDelete = async () => {
    try { await Promise.all(checked.map((id) => api.deleteCategory(id))); setChecked([]); await load(); message.success('已删除选中分类，关联应用已归入未分类') } catch (error) { await load(); message.error((error as Error).message) }
  }
  const filteredCategories = categories.filter((category) => `${category.code} ${category.name}`.toLowerCase().includes(keyword.trim().toLowerCase()))

  return <div>
    <div className="page-title"><div><Typography.Title level={3}>控制台分类</Typography.Title><Typography.Text type="secondary">管理工作台分类及显示顺序，优先级越小越靠前</Typography.Text></div><Space>{can('platform.app.unlink') && <Button danger disabled={!checked.length} onClick={() => void batchDelete()}>删除选中</Button>}{can('platform.app.create') && <Button type="primary" onClick={create}>创建</Button>}</Space></div>
    <Input.Search className="list-filter" allowClear placeholder="筛选编码或名称" value={keyword} onChange={(event) => setKeyword(event.target.value)} />
    <Table className="config-table" loading={loading} rowKey="id" columns={[{ title: '编码', dataIndex: 'code', width: 220 }, { title: '名称', dataIndex: 'name', width: 180 }, { title: '优先级', dataIndex: 'priority', width: 120 }, { title: '操作', width: 160, render: (_: unknown, row: DashboardCategory) => <Space>{can('platform.app.write') && <Button type="link" onClick={() => edit(row)}>编辑</Button>}{can('platform.app.unlink') && <Popconfirm title="删除后关联应用将归入未分类，确认删除？" onConfirm={() => void remove(row)}><Button type="link" danger>删除</Button></Popconfirm>}</Space> }]} dataSource={filteredCategories} scroll={{ x: 680 }} rowSelection={{ selectedRowKeys: checked, onChange: (keys) => setChecked(keys as number[]), getCheckboxProps: () => ({ disabled: !can('platform.app.unlink') }) }} pagination={{ pageSize: 20, showSizeChanger: true, pageSizeOptions: [10, 20, 50], showTotal: (total) => `共 ${total} 条` }} />
