...
export default function UserConfig() {
  ...
  return <div><div className="page-title"><div><Typography.Title level={3}>用户管理</Typography.Title><Typography.Text type="secondary">维护用户账号、头像、权限组与工作台访问权限</Typography.Text></div>{can('platform.user.create') && <Button type="primary" icon={<PlusOutlined />} onClick={create}>创建</Button>}</div><Input.Search className="list-filter" allowClear placeholder="筛选账号或显示名称" value={keyword} onChange={(event) => setKeyword(event.target.value)} /><Table className="config-table" loading={loading} rowKey="id" columns={columns} dataSource={filteredUsers} scroll={{ x: 820 }} pagination={{ pageSize: 50 }} onRow={(record) => ({ onClick: (event) => { if ((event.target as HTMLElement).closest('button,.ant-popover')) return; if (can('platform.user.write')) edit(record) } })} /><Drawer title={selected ? '编辑用户' : '创建用户'} width={440} open={open} onClose={() => setOpen(false)} destroyOnClose extra={<Button type="primary" onClick={() => form.submit()}>保存</Button>}><Form form={form} layout="vertical" onFinish={(values) => void save(values)} autoComplete="off"><Form.Item name="uuid" label="UUID"><Input readOnly placeholder="创建后自动生成" onClick={() => void copyToClipboard(selected?.uuid, 'UUID')} /></Form.Item><Form.Item name="code" label="账号" rules={[{ required: true }]}><Input readOnly={!!selected} autoComplete="off" onClick={() => { if (selected) void copyToClipboard(selected.code, '账号') }} /></Form.Item><Form.Item name="name" label="显示名称" rules={[{ required: true }]}><Input autoComplete="off" /></Form.Item><Form.Item name="avatar" label="头像地址" extra="支持图片 URL；未设置时聊天中显示默认头像"><Input placeholder="https://..." /></Form.Item><Form.Item name="password" label={selected ? '新密码（留空不修改）' : '密码'} rules={selected ? [] : [{ required: true }]}><Input.Password autoComplete="new-password" /></Form.Item><Form.Item name="role" label="兼容权限类别" rules={[{ required: true }]}><Select options={Object.entries(roleLabels).map(([value, label]) => ({ value, label }))} disabled={selected?.code === 'admin'} /></Form.Item><Form.Item name="groupIds" label="权限组" extra="权限组决定菜单、CRUD 和后续数据范围；可多选"><Select mode="multiple" options={groups.map((group) => ({ value: group.id, label: `${group.name} (${group.code})` }))} /></Form.Item></Form></Drawer></div>
}
