...
影视库前台搜索框位于电影、电视剧等分类标签上方，搜索默认覆盖数据库中全部已存影视；电影、电视剧、动漫仅作为分类筛选标签，地区、类型和年份通过可展开/收起的“筛选”面板组合过滤，地区和类型使用按钮多选。新增“最近上新”标签按数据库 `created_at` 入库时间倒序展示资源，搜索条件、筛选条件、排序方式和页码都会保存在地址栏。列表固定每页显示 20 条，分类和搜索变更时自动回到第一页。影视详情地址使用随机公开 UUID，不直接暴露数据库自增 ID。榜单状态仅用于默认排序，不再限制前台搜索和筛选；因此历史上已入库但当前未上榜的影视仍可被检索。
